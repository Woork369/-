document.addEventListener('DOMContentLoaded', () => {
    // 1. НАСТРОЙКИ СЕРВЕРА "ПОСЛАННИК-ЛЕТО"
    ...
    // Элементы интерфейса формы авторизации
    ...
    if (authForm) {
        authForm.addEventListener('submit', (e) => {
            ...
            // Валидация 3: Проверка на простые пароли
            ...
            if (currentMode === 'register') {
                ...
                .then(res => {
                    if (!res.ok) throw new Error("Сервер вернул ошибку при проверке имени.");
                    return res.json();
                })
                .then(users => {
                    if (users.length > 0) {
                    ...
                    // Если имя свободно — создаем запись в таблице 'user'
                ...
                .then(res => {
                    if (!res.ok) throw new Error("Сервер не отвечает на запрос авторизации.");
                    return res.json();
                })
                .then(users => {
                    if (users.length === 0) {
                    ...
                    // Получаем первого найденного пользователя из массива Supabase

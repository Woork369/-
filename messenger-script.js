document.addEventListener('DOMContentLoaded', function() {
    ...
    // Константная графика оригинального SVG Telegram
    ...
    // Кнопки медиазоны
    const globalClipBtn = document.getElementById('global-clip-btn');
    ...
    function openChatWorkspace(chatName) {
        ...
        // КАНАЛЫ открыты для чтения абсолютно всем! Пускаем без проверки инвайтов.
        ...
        // Для групп оставляем клиентскую валидацию приватности по сообщениям
        ...
        .then(messages => {
            ...
            if (!hasAccess) {
                if (fallbackNotice) { fallbackNotice.textContent = "🔒 Вы не являетесь участником этой группы. Доступ запрещен."; fallbackNotice.style.display = 'block'; }
    ...
    function proceedOpeningWorkspace(chatName) {
        ...
        // Шестеренка настроек ТОЛЬКО в группах/каналах и ТОЛЬКО для админа
        ...
        // БОЛЬШОЙ ФИКС: Управление видимостью зоны ввода на каналах без поломки JS обработчиков
        if (currentChatType === 'channel' && activeChatCreator !== currentActiveUser) {
            // Обычный подписчик на канале — полностью скрываем строку ввода и медиа кнопки
            inputZone.style.display = 'none';
        } else {
            // Личные чаты, группы или если ты админ канала — всё доступно в стандартном режиме
        ...
        if (currentChatType === 'contact') {
            ...
            statusTextEl.textContent = 'обновление...';
        ...
        .then(messages => {
            ...
            if (messages.length > lastMessagesCount && lastMessagesCount > 0) {
                ...
                if (lm.sender !== currentActiveUser && document.hidden && window.Notification && Notification.permission === "granted") {
                    new Notification("Лето Мессенджер", { body: "@" + lm.sender + ": " + (lm.type==='text'?lm.text:'📎 Медиафайл'), tag: "leto" });
                }
            }
    ...
    // Изолированные события контекстного меню (ПК и Смартфоны) для всех типов медиафайлов
    ...
    document.getElementById('ctx-delete-msg').addEventListener('click', function() {
        if (!selectedMsgData || !confirm("Удалить это сообщение для всех участников?")) return;
    ...
    // --- МОНОЛИТНАЯ СТАЦИОНАРНАЯ ПАНЕЛЬ НАСТРОЕК КОМНАТ ---
    ...
    // ОРИГИНАЛЬНАЯ ЗОНА СКРЕПКИ И МИКРОФОНА — ПОЛНЫЙ И БЕЗОТКАЗНЫЙ ВОЗВРАТ РАБОТЫ ДЛЯ ЛИЧНЫХ ЧАТОВ И ГРУПП
    ...
    if (globalClipBtn) {
        globalClipBtn.addEventListener('click', function(e) {
            ...
            if (!isRecording) {
                navigator.mediaDevices.getUserMedia({ audio: true }).then(function(stream) {
                    ...
                    mediaRecorder.addEventListener("stop", function() {
                        ...
                        fetch(MY_SUPABASE_URL + "/storage/v1/object/media/" + audioFileName, {
                        ...
                        }).then(() => { lastMessagesCount = 0; loadMessages(); }).catch(err => alert(err.message));
                        stream.getTracks().forEach(track => track.stop());
                    });
                    mediaRecorder.start();
                    isRecording = true;
                    globalMicBtn.textContent = "🛑";
                    globalMicBtn.style.color = "red";
                }).catch(err => alert("Микрофон недоступен: " + err.message));
            } else {
                if (mediaRecorder) mediaRecorder.stop();
                isRecording = false;
                globalMicBtn.textContent = "🎤";
                globalMicBtn.style.color = "";
            }
        });
    }
    ...
    mediaFileInput.addEventListener('change', function() {
        ...
        fetch(MY_SUPABASE_URL + "/storage/v1/object/media/" + fileName, {
            ...
            return fetch(SUPABASE_MSG_URL, {
                ...
                headers: { "apikey": MY_SERVER_KEY, "Authorization": "Bearer " + MY_SERVER_KEY, "Content-Type": "application/json" },
                body: JSON.stringify({
                    sender: currentActiveUser, receiver: activeChatTarget, text: MY_SUPABASE_URL + "/storage/v1/object/public/media/" + fileName,
                    type: isImg ? "image" : "file", chat_type: currentChatType, created_at: new Date().toISOString()
                })
            });
        }).then(() => { lastMessagesCount = 0; loadMessages(); }).catch(err => alert(err.message));
        this.value = "";

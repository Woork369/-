document.addEventListener('DOMContentLoaded', function() {
        ...
        mainDynamicActionBtn.addEventListener('click', function() {
            if (currentChatType === 'contact') {
                ...
                fetch(MY_SUPABASE_URL + "/rest/v1/user?username=eq." + encodeURIComponent(targetUsername.trim().toLowerCase()), {
                    ...
                    if (users.length === 0) { alert("Юзер не найден!"); return; }
                    return fetch(SUPABASE_MSG_URL, {
                ...
                document.getElementById('modal-title').textContent = currentChatType === 'group' ? "Создание группы" : "Создание канала";
                createRoomModal.style.display = 'flex';
            }
        });
    }

    document.getElementById('modal-cancel-btn').addEventListener('click', () => createRoomModal.style.display = 'none');
    document.getElementById('modal-submit-btn').addEventListener('click', function() {
        const rName = document.getElementById('modal-room-name').value.trim();
        const rDesc = document.getElementById('modal-room-desc').value.trim();
        if (!rName) return;

        fetch(SUPABASE_ROOMS_URL, {
            method: "POST", headers: { "apikey": MY_SERVER_KEY, "Authorization": "Bearer " + MY_SERVER_KEY, "Content-Type": "application/json" },
            body: JSON.stringify({ name: rName, description: rDesc, type: currentChatType, creator: currentActiveUser })
        }).then(() => { createRoomModal.style.display = 'none'; loadChats(); openChatWorkspace(rName); });
    });
    ...
    if (roomSettingsBtn) {
        roomSettingsBtn.addEventListener('click', function() {
            ...
            document.getElementById('group-add-users-section').style.display = currentChatType === 'group' ? 'flex' : 'none';
            messagesScreen.style.display = 'none'; inputZone.style.display = 'none'; settingsPanel.style.display = 'flex';
        });
    }

    document.getElementById('close-room-settings-btn').addEventListener('click', function() {
        settingsPanel.style.display = 'none'; messagesScreen.style.display = 'flex'; inputZone.style.display = 'block';
    });
    ...
    document.getElementById('submit-add-member-btn').addEventListener('click', function() {
        const user = document.getElementById('add-member-username').value.trim().toLowerCase();
        if (!user) return;
        ...
        }).then(() => { alert("Добавлен!"); document.getElementById('add-member-username').value = ""; });
    });
    ...
    if (mobileBackBtn) {
        mobileBackBtn.addEventListener('click', () => { appContainer.classList.remove('show-chat'); activeChatTarget = null; });
    }

document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentChatType = 'contact'; // 'contact', 'group', 'channel'
    ...
    function openChatWorkspace(chatName) {
        ...
        // ТОТАЛЬНЫЙ ФИКС БАГА БЕЗОПАСНОСТИ: 
        // Принудительно запрашиваем точные данные о комнате из базы данных ДО выполнения любой логики отображения интерфейса!
        if (currentChatType === 'channel' || currentChatType === 'group') {
            const fetchUrl = SUPABASE_ROOMS_URL + "?name=eq." + encodeURIComponent(chatName);
            fetch(fetchUrl, { method: "GET", headers: { "apikey": MY_SERVER_KEY, "Authorization": "Bearer " + MY_SERVER_KEY } })
            .then(res => res.json())
            .then(rooms => {
                if (rooms && rooms.length > 0) {
                    activeChatCreator = rooms[0].creator; // Перезаписываем железно актуального создателя из БД
                } else {
                    activeChatCreator = null;
                }
                verifyAccessAndOpen(chatName);
            })
            .catch(() => {
                verifyAccessAndOpen(chatName);
            });
    ...
    function verifyAccessAndOpen(chatName) {
        ...
        .then(messages => {
            ...
            const hasAccess = !messages || messages.length === 0 || messages.some(msg => 
                (msg.sender === activeChatCreator && msg.text && msg.text.trim().toLowerCase() === expectedCommand) || (msg.sender === currentActiveUser)
            );

            if (!hasAccess) {
                if (fallbackNotice) { 
                    fallbackNotice.textContent = "🔒 Вы не являетесь участником группы. Доступ запрещен."; 
    ...
    function proceedOpeningWorkspace(chatName) {
        ...
        // Поиск элементов по ID и по классам, чтобы наверняка их скрыть
        const clipBtn = document.getElementById('chat-clip-file-btn') || document.querySelector('.input-container-row .icon-btn:first-child');
        const micBtn = document.getElementById('mic-record-btn');

        // ЖЕЛЕЗОБЕТОННЫЙ БАН: Если это канал и текущий юзер НЕ является создателем
        ...
        if (currentChatType === 'contact' && statusTextEl) {
            ...
            fetch(SUPABASE_MSG_URL + "?sender=eq." + encodeURIComponent(chatName) + "&order=created_at.desc&limit=1", {
                ...
                let rawDate = new Date(lastMsg.created_at);
                if (isNaN(rawDate.getTime())) {
                    ...
                    if (diffMins < 5) { 
                        ...
                        statusTextEl.textContent = 'был(а) в сети в ' + opponentLastActiveTime.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}); 
    ...
    function loadMessages() {
        ...
        .then(messages => {
            ...
            messages.forEach(msg => {
                ...
                avatarCircle.style.background = `linear-gradient(135deg, hsl(${hash % 360}, 65%, 75%), hsl(${(hash + 60) % 360}, 70%, 60%))`;
            ...
            if (isUserAtBottom || messagesScreen.children.length <= 1) { 
    ...
    // --- ЗАПИСЬ ГОЛОСА ---
    const micBtnNode = document.getElementById('mic-record-btn');
            ...
            if (!isRecording) {
                navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
                    ...
                    mediaRecorder.addEventListener("stop", () => {
                        ...
                        micBtnNode.innerHTML = "&#8203;"; 
                    });
                    mediaRecorder.start();
                    isRecording = true; 
                    micBtnNode.textContent = "🛑"; 
        ...
        mainDynamicActionBtn.addEventListener('click', function() {
            if (currentChatType === 'contact') {
                ...
                fetch(SUPABASE_MSG_URL, {
                    ...
                    body: JSON.stringify({ sender: currentActiveUser, receiver: targetUsername.trim().toLowerCase(), text: "👋 Диалог начат.", type: "text", chat_type: "contact", created_at: new Date().toISOString() })
                }).then(() => { loadChats(); });
            } else {
                const modalTitleEl = document.getElementById('create-room-modal-title') || document.getElementById('modal-title');
    ...
    function handleSendMessage() {
        ...
        }).then(() => { mainMessageField.value = ""; lastMessagesCount = -1; loadMessages(); });
    }

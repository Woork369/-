document.addEventListener('DOMContentLoaded', () => {
    ...
    // Режим фильтрации чатов по умолчанию ('contact', 'group', 'channel')
    let currentChatType = 'contact';

    // Имя активного открытого чата в правой панели
    let activeChatTarget = null;

    // Таймер для фонового автообновления сообщений
    let refreshInterval = null;

    // Читаем текущего пользователя из локального хранилища безопасности
    const currentActiveUser = localStorage.getItem('leto_active_user');

    // Если пользователь не залогинен — принудительно возвращаем его на форму входа
    ...
    // Выводим имя вошедшего аккаунта в шапку профиля
    if (activeUsernameEl) {
        activeUsernameEl.textContent = currentActiveUser;
    }

    // --- ЛОГИКА ЗАГРУЗКИ СПИСКА ЧАТОВ ЧЕРЕЗ СЕРВЕР ---
    function loadChats() {
    ...
    // Переключение режимов фильтрации по кнопкам
    ...
    function openChatWorkspace(chatName) {
        ...
        // Запуск интервала фонового обновления
    ...
    // --- ЛОГИКА ЧТЕНИЯ СООБЩЕНИЙ С СЕРВЕРА ---
    ...
    // Слушатели отправки сообщения по клику и кнопке Enter
    if (sendBtn) {
    ...
    // Автоматическая инициализация первого экрана при входе
    loadChats();

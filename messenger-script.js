document.addEventListener('DOMContentLoaded', function() {
    ...
    // Надежная сборка через классическое сложение
    ...
    function loadChats() {
        ...
        if (currentChatType === 'contact') {
            ...
            fetch(url, { method: "GET", headers: { "apikey": MY_SERVER_KEY, "Authorization": "Bearer " + MY_SERVER_KEY, "Cache-Control": "no-cache" } })
            .then(res => res.json())
            .then(messages => {
        ...
        if (currentChatType === 'contact') {
            ...
            .then(users => {
                ...
                return fetch(SUPABASE_MSG_URL, {
                    ...
                    body: JSON.stringify({ sender: currentActiveUser, receiver: trimmedName, text: "", type: "text", chat_type: "contact", created_at: new Date().toISOString() })
                });
            })
            .then((res) => { 
    ...
    if (modalCancelBtn) modalCancelBtn.addEventListener('click', function() { createRoomModal.style.display = 'none'; });
    ...
    modalSubmitBtn.addEventListener('click', () => {
        ...
        if (avatarFiles && avatarFiles.length > 0) {
            ...
            .then(res => {
                if (!res.ok) throw new Error("Ошибка Storage.");
                return res.json();
            })
            .then(() => {
            ...
            .catch(err => alert("Ошибка картинки: " + err.message));
        } else {
    ...
    // ЖИВАЯ СИНХРОНИЗАЦИЯ БЕЗ ПЕРЕЗАГРУЗКИ (ИСПРАВЛЕНО)
    function loadMessages() {
        if (!activeChatTarget || !messagesScreen) return;
        
        let url = "";
        if (currentChatType === 'contact') {
            url = SUPABASE_MSG_URL + "?chat_type=eq.contact&or=(and(sender.eq." + encodeURIComponent(currentActiveUser) + ",receiver.eq." + encodeURIComponent(activeChatTarget) + "),and(sender.eq." + encodeURIComponent(activeChatTarget) + ",receiver.eq." + encodeURIComponent(currentActiveUser) + "))&order=created_at.asc";
        } else {
            url = SUPABASE_MSG_URL + "?chat_type=eq." + currentChatType + "&receiver=eq." + encodeURIComponent(activeChatTarget) + "&order=created_at.asc";
        }

        fetch(url, { 
            method: "GET", 
            headers: { "apikey": MY_SERVER_KEY, "Authorization": "Bearer " + MY_SERVER_KEY, "Cache-Control": "no-cache" } 
        })
        .then(res => res.json())
        .then(messages => {
            if (!messages || !Array.isArray(messages)) return;

            const currentBubblesCount = messagesScreen.querySelectorAll('.msg-bubble').length;
            const visibleMessages = messages.filter(msg => msg.text !== "");

            if (visibleMessages.length !== currentBubblesCount) {

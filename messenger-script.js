document.addEventListener('DOMContentLoaded', () => {
    ...
    // ГЛАВНАЯ ФУНКЦИЯ ВЫВОДА СПИСКОВ ЧАТОВ
    function loadChats() {
        if (!chatsListTarget) return;

        if (currentChatType === 'contact') {
            const url = `${SUPABASE_MSG_URL}?chat_type=eq.contact&or=(sender.eq.${currentActiveUser},receiver.eq.${currentActiveUser})`;
    ...
    // СЦЕНАРИИ НАЖАТИЯ НА ВЕРХНЮЮ КНОПКУ АКТИВНОСТИ
    mainDynamicActionBtn.addEventListener('click', () => {
        if (currentChatType === 'contact') {
            ...
            fetch(`${MY_SUPABASE_URL}/rest/v1/user?username=eq.${encodeURIComponent(trimmedName)}`, { method: "GET", headers: { "apikey": MY_SERVER_KEY, "Authorization": `Bearer ${MY_SERVER_KEY}` } })
            .then(res => res.json())
            .then(users => {
    ...
    modalSubmitBtn.addEventListener('click', () => {
        ...
        if (avatarFiles && avatarFiles.length > 0) {
            ...
            .then(() => saveNewRoom(roomName, roomDesc, `${MY_SUPABASE_URL}/storage/v1/object/public/media/${fileName}`));
        } else {
    ...
    // ЛОГИКА АКТИВАЦИИ РАБОЧЕЙ ОБЛАСТИ (С УЧЕТОМ ДВИЖЕНИЯ НА МОБИЛКАХ)
    function openChatWorkspace(chatName) {
        ...
        // Адаптивная анимация: выкатываем чат поверх списка на мобилках
        if (appContainer) appContainer.classList.add('show-chat');

        // Ограничение прав для каналов
    ...
    function loadMessages() {
        ...
        .then(messages => {
            ...
            messages.forEach(msg => {
                ...
                const senderTag = (currentChatType !== 'contact') ? `<strong style="display:block;font-size:11px;color:var(--primary-color);margin-bottom:2px;">${msg.sender}:</strong>` : '';
